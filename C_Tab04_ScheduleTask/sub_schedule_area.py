...
class ScheduleArea(tk.Frame):
    ...
    def set_widgets(self):
        self.w = OrderedDict()
        self.w["canvas"] = tk.Canvas(self, bg=self.GP.schedule_bg_color, width=20000, height=1000)
    ...
    def draw_corner(self):
        ...
        # self.w["canvas"].create_text(1000, 1000, text="end")
    ...
    def draw_single_project_schedule(self, p_id, y0, schedule_items):
        ...
        self.w["canvas"].create_line(self.x00, y0, self.x00 + 1000, y0,
                                     ...
                                     width=self.dy + 2)
        self.w["canvas"].create_text(self.x00, y0, 
                                     text=f"{ds_p['Name']}",
                                     anchor = "w",
        ...
        for x0, x1, idx in schedule_items:
            ...
            text = f"-{ds['Name']}" + hours + due_date
            self.w["canvas"].create_line(x0 + self.line_left_space, y0, x1 + self.line_left_space, y0, 
                                         fill=self.color_dict[ds["Color"]], 
                                         width=self.dy)
            self.w["canvas"].create_text(x1 + self.line_left_space, y0, text=text, 
                                         anchor="w",
                                         font=(self.GP.font_family, self.GP.font_size))
        return y0 + self.dy * 2
    ...
    def get_draw_items(self):
        ...
        # [ ] task以外の場合 所有者が自分でない場合もあるので対応が必要
